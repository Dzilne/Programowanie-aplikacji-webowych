...
connection.connect((err)=>{
    ...
    {
        console.log('Polaczono z serwerem SQL w api')
    }
})
...
const notFoundError = {
    "Error": 404,
    "Message": "No data here son (L Bozo)"
}

router.get('/', (req, res)=>{
    const apiInfo = {
        "/api" : "Informacje o adresach url API.",
        "/api/students": "Lista uczniów z ich informacjami.",
        "/api/students/:id": "Informacje o wybranym uczniu wg id.",
        "/api/subjects": "Lista przedmiotów szkolnych.",
        "/api/subjects/:id": "Informacje o wybranym przedmiocie sortowane po id."
    }
    res.json(apiInfo)
})
